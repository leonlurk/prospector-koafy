...
client.on('message', async (message) => {
  // <<< ADDED: Ignore messages from status broadcast >>>
  if (message.from === 'status@broadcast') {
      console.log(`[Worker ${userId}][MESSAGE EVENT v2] Ignorando mensaje de status@broadcast.`);
      return; // No procesar nada más para estados
  }
  // <<< END: Ignore status broadcast >>>
...
async function buildPromptWithHistory(chatId, currentMessage, persona) {
    try {
        ...
        console.log(`[Worker ${userId}][CONTEXT] Usando config de agente para buildPrompt:`, JSON.stringify(persona, null, 2));
