...
const EditColumnModal = ({ isOpen, onClose, onSubmit, isLoading, column, boardName }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const newStageType = stageType || null;

    if (newName && (newName !== column.name || newStageType !== (column.stageType || null))) {
      onSubmit(column.id, newName, newStageType);
    } else if (newName === column.name && newStageType === (column.stageType || null)) {
      onClose(); // No change
    } else if (!newName) {
      console.warn("EditColumnModal - Nombre de la columna no puede estar vacío");
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm font-['Poppins']">
      <div className="bg-slate-50 rounded-xl p-6 sm:p-8 w-full max-w-lg shadow-2xl transform transition-all">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-2xl font-bold text-slate-800">Editar Columna</h2>
          <button 
            onClick={onClose} 
        ...
        {boardName && <p className="text-sm text-slate-500 mb-6">En tablero: <span className='font-semibold text-slate-700'>{boardName}</span></p>}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Contacto Realizado"
          ...
          <div>
            ...
            <p className='text-xs text-slate-500 mt-1'>Cambiar el tipo puede afectar cómo se calculan las estadísticas.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-2">
            ...
            <button
              ...
              disabled={isLoading || (!columnName.trim() || (columnName.trim() === column.name && (stageType || null) === (column.stageType || null)))}
            >
              {isLoading ? (
