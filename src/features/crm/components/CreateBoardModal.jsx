...
const CreateBoardModal = ({ isOpen, onClose, onSubmit, isLoading, errorMessage }) => {
  const [boardName, setBoardName] = useState("");

  // Reset el nombre del tablero cuando se cierra el modal
  useEffect(() => {
    if (!isOpen) {
      setBoardName("");
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (boardName.trim()) {
      ...
      // Consider using a more integrated error display than alert
      // For now, let's assume parent handles detailed error via errorMessage prop if needed
      console.warn("CreateBoardModal - Nombre del tablero vacío");
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm font-['Poppins']">
      <div className="bg-slate-50 rounded-xl p-6 sm:p-8 w-full max-w-lg shadow-2xl transform transition-all">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-slate-800">Crear Nuevo Tablero</h2>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Leads de Enero, Proyecto Alpha"
